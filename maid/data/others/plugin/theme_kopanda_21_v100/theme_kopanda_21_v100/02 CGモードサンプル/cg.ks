...
tf.cg_posx = [150, 480, 810]; // サムネイルのX座標
tf.cg_posy = [210, 400];      // サムネイルのY座標
tf.cg_thumbnail_width  = 320; // サムネイルの幅
tf.cg_thumbnail_height = 180; // サムネイルの高さ

// 前ページボタン
tf.btn_prev_pos    = [48, 348]; // x, y
tf.btn_prev_width  = 48;
tf.btn_prev_height = 96;
tf.btn_prev_img    = ["append_theme/gallery_prev.png", "append_theme/gallery_prev2.png"];

// 次ページボタン
tf.btn_next_pos    = [1184, 348]; // x, y
tf.btn_next_width  = 48;
tf.btn_next_height = 96;
...
[macro name="pagination"]
  ...
  [ptext layer="0" name="pagination" text="&tf.page + 1 + '/3'" x="0" y="645" size="18" color="0x443836" width="1280" align="center"]
  [endmacro]
...
; 次ページへ
[button hint="次のページ" graphic="&tf.btn_next_img[0]" enterimg="&tf.btn_next_img[1]" target="*nextpage" x="&tf.btn_next_pos[0]" y="&tf.btn_next_pos[1]"]
...
; 前のページへ
[button hint="前のページ" graphic="&tf.btn_prev_img[0]" enterimg="&tf.btn_prev_img[1]" target="*backpage" x="&tf.btn_prev_pos[0]" y="&tf.btn_prev_pos[1]"]
