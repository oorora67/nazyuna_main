...
tf.cg_posx = [225, 517, 809]; // サムネイルのX座標
tf.cg_posy = [240, 413]; // サムネイルのY座標
tf.cg_thumbnail_width  = 272; // サムネイルの幅
tf.cg_thumbnail_height = 153; // サムネイルの高さ
...
[macro name="pagination"]
  ...
  [ptext layer="0" name="pagination" text="&tf.page + 1 + '/3'" x="0" y="640" size="18" color="0xa6917f" width="1280" align="center"]
  [endmacro]
...
; ギャラリーモード終了
[button graphic=append_theme/gallery_close.png enterimg=append_theme/gallery_close2.png target=*backtitle x=0 y=645]
...
; 次ページへ
[button graphic=append_theme/gallery_next.png enterimg=append_theme/gallery_next2.png target=*nextpage x=1157 y=363]
...
; 前のページへ
[button graphic=append_theme/gallery_prev.png enterimg=append_theme/gallery_prev2.png target=*backpage x=103 y=363]
